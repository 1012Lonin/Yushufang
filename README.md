...
flowchart TD
    ...
    subgraph Hanlin["翰林院内部协作"]
        ...
        Xiu -->|spawn| Shu1["庶吉士<br/>文献检索"]
        Zhang -->|spawn| Bian["编修<br/>逐章写作"]
        Bian -->|spawn| Shu2["庶吉士<br/>查阅前文"]
        Zhang -->|spawn| Jian["检讨<br/>同行评审"]
        Jian -.->|评审报告| Zhang
        Zhang -->|spawn| QizhuHanlin["起居注官<br/>过程记录"]
        QizhuHanlin -.->|过程日志| Zhang
    end

    Zhang -->|sessions_send| Xingbu
    Xingbu -->|引用审计报告| Zhang
    ...
    class Zhang,Xiu,Bian,Jian,Shu1,Shu2,QizhuHanlin hanlin
    class Ducha review
...
③ 翰林院掌院内部调度:
   ...
   3d. 并行 spawn:
       · 起居注官: 实时记录创作过程（选题→综述→写作→评审）
       · 掌院 → sessions_send 刑部: "引用审计请求"
         刑部: 12篇引用，3处格式不规范 → 返回审计报告
       掌院修正引用 → 检讨二审 ✅
...
```
                         【皇帝 / 司礼监 sessions_spawn 触发】
                                        │
                                        ▼
                              ┌───────────────────┐
                              │    🎓 掌院学士     │ ←── 强模型 · 600s · sandbox all
                              │  统筹 + 终审调度权  │      可 spawn：修撰/编修/检讨/庶吉士/起居注官
                              └────────┬──────────┘
                         ┌─────────────┼─────────────┬─────────────┐
                         ▼             ▼             ▼             ▼
                  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐
                  │  📝 修撰   │ │  ✍️ 编修  │ │  🔍 检讨  │ │  📚 起居注官│
                  │ 强模型·all │ │ 强模型·all│ │ 快模型·all│ │ 快模型·all │
                  │架构·综述·  │ │ 逐章写作  │ │ 同行评审  │ │ 实时记录·  │
                  │ 人物档案   │ │ ≥10000字/ │ │ 三级标注  │ │ 过程留档   │
                  └─────┬─────┘ └─────┬─────┘ └─────┬─────┘ └─────┬─────┘
                        │ spawn        │ spawn        │             │
                        ▼              ▼              │             │
                  ┌───────────┐ ┌───────────┐       │             │
                  ...
                  └───────────┘ └───────────┘       │             │
                        ...
                        ▼              ▼             ▼             ▼
                  └─────────────────────────────────────────────┘
                        三方输出汇聚：综述 + 初稿 + 评审报告 + 过程日志
                                        │
                                        ▼
                  ┌───────────────┐         ┌─────────────────┐
                  │   掌院终审    │────────▶│   sessions_send │───▶ 刑部
                  │  综合审议     │         │   刑部引用审计   │      (引用审计)
                  └───────────────┘         └─────────────────┘
                                        │
                                        ▼
...
─── 图例 ───────────────────────────────────────────────────────────
│ spawn 关系   │ 调用关系   ───▶ 结果汇报
│ 强模型/all   快模型/all   所有子 agent 均受 sandbox 保护
└─────────────────────────────────────────────────────────────────
```
...
阶段    动作                                         角色         输出
─────────────────────────────────────────────────────────────────────────────
0        接单 + 同步启动过程记录                      掌院学士      任务拆解 + 记录开始
1        架构设计 / 文献综述 / 人物档案               修撰          综述 + 架构
          └─ spawn 外部搜索 / 文献检索                 庶吉士        参考材料
2        逐章写作（≥10000字/章）                      编修          初稿章节
          └─ spawn 查阅前文 / 一致性核对              庶吉士        一致性报告
3        同行评审（三级标注🔴🟡🟢）                  检讨          评审报告
4        并行：引用审计 / 过程留档                    刑部 / 起居注官  审计报告 + 过程日志
5        终审：综合评审 + 审计 + 过程                 掌院学士      终审稿
─────────────────────────────────────────────────────────────────────────────
```

| 步骤 | 角色 | 动作 | 输出 |
|------|------|------|------|
| 0 | 掌院学士 | 接单拆解，同步 spawn 起居注官开始过程记录 | 任务拆解 + 记录开始 |
| 1 | 修撰 → 庶吉士 | 文献综述 / 架构设计 / 人物档案 | 综述 + 架构方案 |
| 2 | 编修 → 庶吉士 | 逐章写作（≥10000字/章），查阅前文确保一致性 | 初稿章节 |
| 3 | 检讨 | 同行评审，三级标注（🔴致命/🟡重要/🟢优化） | 评审报告 |
| 4 | 刑部 + 起居注官 | 并行：引用审计 + 过程实时留档 | 审计报告 + 过程日志 |
| 5 | 掌院学士 | 综合评审 + 审计结果 + 过程日志终审 | 终审稿 → 返回司礼监 |
