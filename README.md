...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/版本-1.0.1-red?style=for-the-badge" />
</p>
...
└────────────────────────────┬────────────────────────────────────────┘
   ...
   返回Prompt+Plan        │  + 手动@
        ...
        │ 礼部(文案)   │
        │ 工部(运维)   │  cron: 每 2h 巡检
        │ 吏部(知识库) │  其他 agent 直接读取其 workspace
        │ 刑部(法务)   │ 翰林院调用: 引用审计
        └──────┬───────┘
               │ sessions_spawn
               ▼
...
户部采用 **Shell 脚本（零 LLM）+ Agent 报告（固定 LLM）**混合架构：

```
每 30 分钟: Shell 脚本（hubu-data-collect.sh，零 LLM 调用）
  ↓ curl 本地 OpenClaw API 获取 token 用量
  ↓ 解析 → 写入 workspace/data/api-usage.json
  ↓ 阈值检查:
     < 月供 50% → 只记日志
     ≥ 月供 50% → 标记，等日报触发分析
     ≥ 月供 80% 或超日硬限额 → 立即告警（发消息给司礼监）

每日 23:00: 户部 Agent 日报（固定 LLM 调用）
  ↓ 读取 api-usage.json 当日数据
  ↓ 生成结构化日报
...
> Shell 脚本路径由 `hubu-data-collect.sh` 提供，配合系统 crontab `*/30 * * * *` 运行，完全不经过 LLM。
