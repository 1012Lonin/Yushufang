...
# --install-hook 参数：安装 git hook 后直接退出
...
if command -v openclaw &>/dev/null; then
  if openclaw gateway status 2>&1 | grep -qi "running\|running"; then
    echo -e "  ${GREEN}✓${NC} Gateway 运行中"
  elif openclaw gateway status 2>&1 | grep -qi "stopped\|stopped"; then
